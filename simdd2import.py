...
def updateStudentInfo(redID, enrollmentStatus):
	...
	request = urllib2.Request("https://oncampusliving.sdsu.edu/StarRezREST/services/select/entry.xml?id1=" + redID)
	...
	try:
		...
		request = urllib2.Request("https://oncampusliving.sdsu.edu/StarRezREST/services/select/entrycustomfield.xml?entryid=" + entryID + "&customfielddefinitionid=" + enrollmentStatusFieldID)
		base64string = base64.encodestring('%s:%s' % (username, passowrd)).replace('\n', '')
		request.add_header("Authorization", "Basic %s" % base64string)
		try:
			handler = urllib2.urlopen(request)
			...
			request = urllib2.Request("https://oncampusliving.sdsu.edu/StarRezREST/services/update/entrycustomfield/" + fieldID)
